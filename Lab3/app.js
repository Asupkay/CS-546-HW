...
async function processFile(path) {
    ...
    if(path && typeof path == 'string') {
        ...
        try {
            let contents = await fileData.getFileAsJSON(resultPath);
            ...
            try {
                let contents = await fileData.getFileAsString(path + ".txt");
                ...
                try {
                    await fileData.saveStringToFile(path + ".debug.txt", textMetrics.simplify(contents))
                    ...
                    try {
                        await fileData.saveJSONToFile(path + ".result.json", textInfo);
